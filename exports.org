...
I wrote and vented a lot about it in the [[https://beepb00p.xyz/sad-infra.html][previous post]].
...
I suggested a [[https://beepb00p.xyz/sad-infra.html#data_mirror][*'data mirror'*]], a piece of software that continuously syncs/mirrors user's personal data.
...
I will [[https://beepb00p.xyz/myinfra.html][link]] to my infrastructure map throughout the post, hopefully you'll enjoy exploring it. Links will point at specific clusters of the map and highlight them, so hopefully it will be helpful in communicating the design decisions.
...
It might not be that hard to achieve for one particular data source, but when you want to use [[https://beepb00p.xyz/my-data.html][ten or more]],
...
- [[#export_layer][export layer]]: knows how to get your data from the silos
  ...
  The purpose of the export layer is to reliably fetch and serialize raw data on your disk. It roughly corresponds to the concept of the [[https://beepb00p.xyz/sad-infra.html#data_mirror]['data mirror app']]. 

  Export scripts deal with the tedious business of authorization, pagination, being tolerant of network errors, etc.
  (: [[https://beepb00p.xyz/myinfra.html#exports][exports]])
...
- [[#dal][data access layer (DAL)]]: knows how to read your data
  ...
  (: [[https://beepb00p.xyz/myinfra.html#dal][data access layer]])
...
- downstream data consumers
  ...
  (: [[https://beepb00p.xyz/myinfra.html#mypkg][my.]])
  ...
  For me, it's manifested as [[https://beepb00p.xyz/sad-infra.html#mypkg][a Python package]]. I can simply import it in any Python script, and it knows how to read and access any of my data.
...
Here, I won't elaborate much on potential difficulties during exports, as I wrote about them [[https://beepb00p.xyz/sad-infra.html#exports_are_hard][before]].
...
They can be useful to get the 'initial' bit of your data, past the [[https://beepb00p.xyz/sad-infra.html#data_is_vanishing][API limits]].
...
- disadvantages
  - *might be impossible* due to [[https://beepb00p.xyz/sad-infra.html#data_is_vanishing][API restrictions]]
...
- disadvantages
  - potentially very error-prone, harder to implement
    - if you're not careful with [[https://beepb00p.xyz/sad-infra.html#pagination][pagination]] and misinterpret documentation you might never request some data
    - if you're not careful with [[https://beepb00p.xyz/sad-infra.html#consistency][transactional logic]], you might leave your export in an inconsistent and corrupt state
...
Otherwise you inevitably miss some data and in the best case scenario have to get it [[#export_manual][manually]], or in the worst case [[https://beepb00p.xyz/./takeout-data-gone.html][lose it forever]].
...
- Reddit API is restricted to 1000 items, so synthetic logic is used in [[https://github.com/karlicoss/rexport/blob/874e6116bfba8cbd63fa3b4d93810a1488cb8464/dal.py#L136][rexport]] tool
  ...
  I elaborate on Reddit [[https://beepb00p.xyz/unnecessary-db.html#example_reddit][here]].

- Chrome only keeps 90 days of browsing history in its database

  [[https://beepb00p.xyz/unnecessary-db.html#chrome_dal][Here]] I write in detail about why synthetic exports make a lot of sense for Chrome.

* Export layer
Map: [[https://beepb00p.xyz/myinfra.html#exports][export layer]].
...
Just a quick reminder of [[https://beepb00p.xyz/sad-infra.html#exports_are_hard][the problems]] that we're dealing with:
...
- don't be overly defensive, or allow to [[https://beepb00p.xyz/mypy-error-handling.html#global_policy][configure]] non-defensive behavior
...
I'm elaborating on this point [[https://beepb00p.xyz/unnecessary-db.html#asis][here]].
...
Map: [[https://beepb00p.xyz/myinfra.html#fs][filesystem]].
...
Map: [[https://beepb00p.xyz/myinfra.html#dal][data access layer]].
...
- you can still use caching. Just make sure it doesn't involve boilerplate or cognitive overhead to use. E.g. [[https://beepb00p.xyz/unnecessary-db.html#cachew_cachew][cachew]].
...
- [[https://beepb00p.xyz/unnecessary-db.html#example_reddit][Reddit]]
- [[https://beepb00p.xyz/unnecessary-db.html#relational][Instapaper/Endomondo]]
- [[https://beepb00p.xyz/unnecessary-db.html#maintaining][Pocket]]
- [[https://beepb00p.xyz/unnecessary-db.html#example_chrome][Chrome]]
...
I wrote a whole [[https://beepb00p.xyz/scheduler.html][post]] on scheduling and job running with respect to the personal infrastructure. In short:

- on desktop: at the moment, I'm mostly using cron (to be more specific, [[https://beepb00p.xyz/scheduler.html#fcron][fcron]]).

  I'm still [[https://beepb00p.xyz/scheduler.html#solution][thinking]] of an alternative, but overall using cron is okay.

- on Android phone: [[https://beepb00p.xyz/scheduler.html#phone][I'm using Automate app and cron]]
